...
class SubmissionConformity(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("submission_id", "form_field_id", name="uq_submission_conformities_submission_field"),
        CheckConstraint("status IN ('conforme', 'nao_conforme')", name="ck_submission_conformities_status"),
    ...
    form_field_id: Mapped[str] = mapped_column(ForeignKey("form_fields.id", ondelete="CASCADE"), nullable=False)
