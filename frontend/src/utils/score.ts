...
export function scoreColorVar(score: number): string {
  return score >= SCORE_THRESHOLD_OK ? 'var(--sa-ok)' : score >= SCORE_THRESHOLD_WARN ? 'var(--sa-warn)' : 'var(--sa-danger)'
}

export function scoreChipClass(score: number): string {
  return score >= SCORE_THRESHOLD_OK ? '' : score >= SCORE_THRESHOLD_WARN ? 'status-chip--warn' : 'status-chip--inactive'
}

export function scoreText(score: number): string {
  return score >= SCORE_THRESHOLD_OK ? 'Aprovado' : score >= SCORE_THRESHOLD_WARN ? 'Atenção' : 'Reprovado'
}
